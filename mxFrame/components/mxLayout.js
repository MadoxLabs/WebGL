...
//    public List<mxPlaceable> mObjects = new List<mxPlaceable>();
    ...
    Arrange()
    {
      ...
      Rectangle winSize = mWidget.Rect;

      foreach (mxPlaceable layout in mObjects)
      {
        Rectangle objSize = layout.Rect;
        Rectangle objLayout = new Rectangle(winSize.X, winSize.Y, objSize.Width, objSize.Height);

        Rectangle rect;

        // if the size is fixed, lets set that now so it can be used in placement
        if (layout.mBottomRight.xPlace == mxPlace.Fixed)
          objSize.Width = layout.mBottomRight.mOffset.x;
        if (layout.mBottomRight.yPlace == mxPlace.Fixed)
        ...
        switch (layout.mTopLeft.xPlace)
        {
          case mxPlace.LeftOf:
            ...
            break;
          case mxPlace.Fixed:
          case mxPlace.LeftAlign:
            if (layout.mTopLeft.xRelativeTo == null)
              objLayout.X = 0;
            else
              objLayout.X = rect.Left;
            break;
          case mxPlace.RightOf:
            objLayout.X = rect.Right;
            break;

          case mxPlace.RightAlign:
            if (layout.mTopLeft.xRelativeTo != null)
